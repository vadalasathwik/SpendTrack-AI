...
// Helper to extract refresh token from cookies or authorization header
...
router.get("/api/auth/google", (req: any, res: any) => {
  try {
...
router.get("/api/auth/google/callback", async (req: any, res: any) => {
  try {
    ...
    // Redirect to frontend app with access token in fragment
...
router.post("/api/auth/refresh", async (req: any, res: any) => {
  try {
    ...
    res.clearCookie("trackpay_refresh_token");
...
router.post("/api/auth/logout-all", async (req: any, res: any) => {
  try {
...
router.get("/api/auth/sessions", async (req: any, res: any) => {
  try {
    const userId = req.user?.userId || req.user?.uid || req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Unauthenticated" });
...
router.delete("/api/auth/sessions/:sessionId", async (req: any, res: any) => {
  try {
    const userId = req.user?.userId || req.user?.uid || req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Unauthenticated" });
